...
public partial class users_screws : System.Web.UI.Page
{
    DataSet ds;
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            DropDownList1.DataSource = itemS.GetScrewTypes();
            DropDownList1.DataTextField = "typeName";
            DropDownList1.DataValueField = "idType";
            DropDownList1.DataBind();

            ds = itemS.GetScrewOnly();
        }
        else
        {
            ds = itemS.GetScrewsByType(DropDownList1.Items[DropDownList1.SelectedIndex].Value);
        } 
